...
ConvLSTM · SimVP · PredRNN-V2 · VideoTransformer 네 모델을 같은 데이터·손실·평가로 비교해
...
# 네 모델 학습 (TensorFlow 가 설치된 인터프리터로 실행). 결과는 results/<Model>/
...
| `videotf_predict_colab.py` | VideoTransformer 엔트리 (factorized space-time attention + hybrid conv head) |
...
| `tests/` | 파이프라인·모델 4종·노트북 생성기·리포트·환경 검사 테스트 |
...
| 알고 싶은 것 | 문서 |
| --- | --- |
| 네 모델 구조·적응 사항·실행·배포 절차 | [doc/model_comparison.md](doc/model_comparison.md) |
